#!/bin/bash
# notion-pending-poster.sh - Stop Hook
# ~/.claude/notion-pending.json이 존재하면 자동으로 Notion 지식 베이스에 POST
...
db_id = "323d1034-8f3f-815b-816d-fb88391f31da"  # 지식 베이스 DB
...
try:
    ...
    if resp.get("object") == "page":
        ...
        # Discord 알림
        agent = tags[0] if tags else "에이전트"
        send_discord(f"📋 **{agent}** 완료 보고\n> {title}\n> 노션 지식 베이스에 기록됨. 확인 후 다음 세션 열어주세요.")
        print(json.dumps({
            "continue": True,
            "systemMessage": f"✅ [노션 자동 기록 완료] '{title}' 지식 베이스 등록 + Discord dev-log 알림 전송."
        }))
    else:
